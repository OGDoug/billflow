...
async function directSQL(sql) {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      headers: {
        'Authorization': `Bearer ${supabaseServiceKey}`,
        'Content-Type': 'application/vnd.pgrst.plan+text; for="application/sql"',
        'Content-Length': Buffer.byteLength(postData),
        'apikey': supabaseServiceKey
      }
    };
...
async function runMigration() {
  console.log('🚀 Creating Duxbill cloud storage tables...');
  ...
  try {
    ...
    console.log('🎉 Pro users now have cloud storage for all their data');
